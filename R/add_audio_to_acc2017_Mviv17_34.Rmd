...
### read audio data
```{r}
load("../../../../Dropbox/Vivesi2018/Analysis/Audio/Mviv17_34_calls.robj")
...
```{r}
m17_34 <- read.csv("C:../../../../Dropbox/VivesiACC/data/clean/clean_ACC_Mviv17_34.csv")
m17_34$datetime <- as.POSIXct(m17_34$time, format = "%Y-%m-%d %H:%M:%OS", tz = "UTC")
...
for(i in 1:nrow(Mv18_main)){
  start_idx <- which.min(abs((m17_34$datetime)-(Mv18_main$Local_Time[i]+
                               Mv18_main$start_time[i])))
  end_idx <- which.min(abs((m17_34$datetime) -(Mv18_main$Local_Time[i]+
                                           Mv18_main$start_time[i]+
                                           Mv18_main$duration[i])))
  # Mv18_main$Local_Time[i]+Mv18_main$start_time[i]
  # m17_34$datetime[1]
  m17_34$call[start_idx:end_idx] <- 1
}
...
range(m17_34$datetime)
m17_34$datetime[start_idx]
```

## get buzz times
```{r}
spec <- "../../../../Dropbox/VivesiAudio/BuzzCheck/Mviv17/Mviv17_34/Buzz/"
...
for(i in 1:length(buzz_times)){
  idx <- which.min(abs(m17_34$datetime-(buzz_times[i]-7*3600)))
  m17_34$buzz[idx:(idx+25)] <- 1
}


with(m17_34, plot(datetime, buzz))
with(m17_34[500000:700000,], 
...
```{r}
spec <- "../../../../Dropbox/VivesiAudio/BuzzCheck/Mviv17/Mviv17_34/AlmostBuzz/"
files <- list.files(path = spec)
...
m17_34$almost_buzz <- 0
i = 1
for(i in 1:length(almost_buzz_times)){
  idx <- which.min(abs(m17_34$datetime-(almost_buzz_times[i]-7*3600)))
  m17_34$almost_buzz[idx:(idx+25)] <- 1
}


with(m17_34, plot(datetime, almost_buzz))
with(m17_34[500000:700000,], 
...
# save data
```{r}
save(m17_34, Mv18_main, Mv18_buzz, Mv18_cons, Mviv18, Mv18_summary, 
     file = "../../../../Dropbox/Vivesi2018/Analysis/Accelerometer/data/Mviv17_34_complete.robj")
load("../../../../Dropbox/Vivesi2018/Analysis/Accelerometer/data/Mviv17_34_complete.robj")

```

# write csv
```{r}
write.csv(m17_34, file = "../../../../Dropbox/VivesiACC/data/clean/Audio/Mviv17_34_ACC_Call.csv")
```
