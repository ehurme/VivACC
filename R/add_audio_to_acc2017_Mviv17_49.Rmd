...
### read audio data
```{r}
load("/Users/Edward/Dropbox/Vivesi2018/Analysis/Audio/Mviv17_49_calls.robj")
...
```{r}
m17_49 <- read.csv("C:/Users/Edward/Dropbox/VivesiACC/data/clean/clean_ACC_Mviv17_49.csv")
m17_49$datetime <- as.POSIXct(m17_49$time, format = "%Y-%m-%d %H:%M:%OS", tz = "UTC")
...
for(i in 1:nrow(Mv18_main)){
  start_idx <- which.min(abs((m17_49$datetime)-(Mv18_main$Local_Time[i]+
                               Mv18_main$start_time[i])))
  end_idx <- which.min(abs((m17_49$datetime) -(Mv18_main$Local_Time[i]+
                                           Mv18_main$start_time[i]+
                                           Mv18_main$duration[i])))
  # Mv18_main$Local_Time[i]+Mv18_main$start_time[i]
  # m17_49$datetime[1]
  m17_49$call[start_idx:end_idx] <- 1
}
...
range(m17_49$datetime)
m17_49$datetime[start_idx]
```

## get buzz times
```{r}
spec <- "../../../../Dropbox/VivesiAudio/BuzzCheck/Mviv17/Mviv17_49/Buzz/"
...
for(i in 1:length(buzz_times)){
  idx <- which.min(abs(m17_49$datetime-(buzz_times[i]-7*3600)))
  m17_49$buzz[idx:(idx+25)] <- 1
}


with(m17_49, plot(datetime, buzz))
with(m17_49[500000:700000,], 
...
```{r}
spec <- "../../../../Dropbox/VivesiAudio/BuzzCheck/Mviv17/Mviv17_49/AlmostBuzz/"
files <- list.files(path = spec)
...
m17_49$almost_buzz <- 0
i = 1
for(i in 1:length(almost_buzz_times)){
  idx <- which.min(abs(m17_49$datetime-(almost_buzz_times[i]-7*3600)))
  m17_49$almost_buzz[idx:(idx+25)] <- 1
}


with(m17_49, plot(datetime, almost_buzz))
with(m17_49[500000:700000,], 
...
# save data
```{r}
save(m17_49, Mv18_main, Mv18_buzz, Mv18_cons, Mviv18, Mv18_summary, 
     file = "../../../../Dropbox/Vivesi2018/Analysis/Accelerometer/data/Mviv17_49_complete.robj")
load("../../../../Dropbox/Vivesi2018/Analysis/Accelerometer/data/Mviv17_49_complete.robj")

```

# write csv
```{r}
write.csv(m17_49, file = "../../../../Dropbox/VivesiACC/data/clean/Audio/Mviv17_49_ACC_Call.csv")
```
