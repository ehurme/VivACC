...
buzz_times <- data.frame(start_clip = rep(ymd_hms("2000-01-01 12:00:00.001"), length(buzz_clips)), 
                                          segment = NA)
...
buzz_times <- read.csv("../../../Dropbox/VivesiACC/data/truebuzz/Mviv17_33_truebuzz.csv")
...
for(i in 1:length(na.omit(buzz_times$true_buzz))){
  true_start <- 
    which.min(abs(Metrics$datetime - (buzz_times$start_clip[i] - 0.25)))
  true_end <- 
    which.min(abs(Metrics$datetime - (buzz_times$start_clip[i] + 0.25)))
